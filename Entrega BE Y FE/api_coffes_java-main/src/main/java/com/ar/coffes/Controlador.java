...
@WebServlet("/cafes") // Anotación que mapea este servlet a la URL
public class Controlador extends HttpServlet { // Declaración de la clase Controlador que extiende HttpServlet
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            Cafe cafe = mapper.readValue(request.getInputStream(), Cafe.class);  // Convertir el JSON de la solicitud a un objeto 
        
            // Consulta SQL para insertar un registro
            String query = "INSERT INTO coffes (titulo, descripcion, imagen) VALUES (?, ?, ?)";
            ...
            statement.setString(1, cafe.getTitulo());
            statement.setString(2, cafe.getDescripcion());
            statement.setString(3, cafe.getImagen());
            ...
            if (rs.next()) {
                Long idcafe = rs.getLong(1);  // Obtener el valor del primer campo generado automáticamente (en este caso, el ID)
                ...
                String json = mapper.writeValueAsString(idcafe);  // Convertir el ID a formato JSON
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            List<Cafe> cafes = new ArrayList<>();  // Crear una lista para almacenar objetos 

            // Iterar sobre cada fila de resultados en el ResultSet
            while (resultSet.next()) {
                // Crear un objeto Cafe con los datos de cada fila
                Cafe Cafe = new Cafe(
                    resultSet.getInt("id_coffe"),
                    resultSet.getString("titulo"),  
                    resultSet.getString("descripcion"),
                    resultSet.getString("imagen")
                );
                cafes.add(Cafe);  // Agregar el objeto  a la lista
            }

            ObjectMapper mapper = new ObjectMapper();  // Crear un objeto ObjectMapper para convertir objetos Java a JSON
            String json = mapper.writeValueAsString(cafes);  // Convertir la lista de cafes a formato JSON
       ...
       protected void doDelete(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            String query = "delete from coffes  where id_coffe =?";
